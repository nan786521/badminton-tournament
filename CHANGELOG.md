...
#### 重要修復
- **平手偵測修正**：`sameRecord()` 加入淨分差比較，同勝負但淨分不同不再誤判為平手，減少不必要的手動勾選
- **彈出視窗防護**：所有列印/報表功能加入 `window.open()` null 檢查，瀏覽器封鎖彈出視窗時顯示提示而非當機
- **決賽出賽單雙打修正**：`generateFinalsSheets()` 正確偵測雙打類別，出賽單顯示雙人欄位
- **html2canvas CDN 統一**：出賽單改用 cdnjs.cloudflare.com（與其他頁面一致），修正離線時無法載入的問題
- **晉級人數防禦**：`getQualifyCount()` 新增 5 人以上組別的安全處理
- **重複結算保護**：`promoteWinners()` 在已有決賽對戰資料時彈出確認對話框，防止誤清資料

#### 程式碼品質
- **抽取 `rankPlayers()`**：統一 6 處重複的勝負排序邏輯為單一函數
- **抽取 `_resetTournamentUI()`**：合併 `createNewEvent()` 與 `clearCurrentData()` 共 ~40 行重複 UI 重置程式碼
- **抽取 `openPrintWindow()`**：統一 6 處 `window.open()` 呼叫的 null 防護
- **新增 `escapeJsInAttr()`**：修正 onclick 屬性中類別名含引號時的 JS 語法錯誤

#### UX 改善
- **決賽分數即時提醒**：決賽分數輸入時若雙方相同，即時顯示 toast 警告（不需等到按確認勝者）
- **搜尋排除棄權標籤**：選手搜尋不再誤匹配「棄」「傷」等棄權標籤文字
- **決賽出賽單含季軍賽**：4 人以上決賽自動產生季軍賽出賽單
...
#### 新功能
- **比分規則驗證**：輸入分數時自動檢查羽球 21 分制規則（超過 30 分上限、20 平後需贏 2 分、正常比賽 21 分），以 toast 提醒（非阻斷）
- **選手搜尋**：預賽計分區頂部新增搜尋框，即時比對所有類別/組別中的選手姓名，自動捲動並高亮匹配位置；按 ESC 清除搜尋
- **Enter 快速跳轉**：在分數欄位按 Enter 跳到下一個欄位、Shift+Enter 跳到上一個，並自動選取方便覆寫
- **列印本組成績**：計算戰績後顯示「列印本組」按鈕，開啟新視窗輸出該組完整戰績表（含列印/PDF 下載）
- **CSV 匯出**：控制台新增「CSV」按鈕，匯出所有預賽及決賽比賽明細（類別、組別、場次、選手、分數、勝者）
...
#### 新功能
- **JSON 備份/還原**：控制台新增「JSON 備份」和「載入備份」按鈕，不需網路即可匯出/匯入完整賽事資料（不依賴 xlsx 函式庫）

#### 改善
- **程式碼目錄索引**：JS 區塊頂部新增目錄索引註解，列出 27 個主要段落與行號，方便開發者導覽
- **手機適配加強**：標題列按鈕在小螢幕自動換行、對戰圖區塊最小寬度縮小，改善 ≤480px 裝置的顯示
...
#### 安全性修復
- **XSS 防護**：所有 innerHTML 中的選手姓名、類別名稱、組別名稱都經過 `escapeXml()` 轉義，防止特殊字元導致 HTML 注入。修改 `alignName()`、`renderBracketMatchHtml()`、`renderBracketResult()` 等函數

#### 效能優化
- **儲存效能**：分數輸入時不再每次按鍵都執行完整序列化+儲存，改為失焦/切換欄位時才存檔，另設 3 秒防抖安全網
- **localStorage 容量監控**：使用量超過 4MB 時主動提醒用戶清理舊賽事或匯出備份，防止存滿後資料遺失
- **Undo 記憶體**：復原堆疊上限從 30 降為 10，減少記憶體佔用

#### 無障礙改善
- Toast 通知加 `role="status" aria-live="polite"`，讓螢幕閱讀器能播報提示訊息
- 進度面板與儲存指示器加 `aria-live="polite"`

#### PDF 列印品質
- 成績公告與綜合報表加 `page-break-inside: avoid`，防止表格行和獎牌區塊在分頁處被截斷
...
#### Bug 修正
- **決賽輪空場次編號修正**：BYE（輪空）不再佔用場次編號
  - 對戰圖 PDF（`buildBracketSVG`）：輪空場次不顯示場次號碼，真實比賽從 `*1` 連續編號
  - 出賽單（`generateFinalsSheets`）：輪空場次不遞增場次編號，避免出賽單號碼跳號
